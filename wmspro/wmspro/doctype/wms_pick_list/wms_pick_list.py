...
class WMSPickList(Document):
    ...
    def assign_to_picker(self, picker_user):
        ...
        if frappe.db.has_column("WMS Pick List Item", "picked_by"):
            for row in self.items:
                frappe.db.set_value(
                    "WMS Pick List Item",
                    row.name,
        ...
        frappe.get_doc({
            "doctype": "Notification Log",
            "subject": f"Pick List {self.name} assigned to you",
            "for_user": picker_user,
            "document_type": "WMS Pick List",
            "document_name": self.name,
            "type": "Alert"
        ...
        frappe.get_doc({
            ...
            "reference_type": "WMS Pick List",
            "reference_name": self.name,
            "description": f"Pick items for Pick List {self.name}",
            "status": "Open"
        }).insert(ignore_permissions=True)

        frappe.publish_realtime("notification", {"user": picker_user})
    ...
    def start_picking(self):
        ...
        if frappe.db.has_column("WMS Pick List Item", "picjed_at"):
            for row in self.items:
                frappe.db.set_value(
                    "WMS Pick List Item",
                    row.name,
    ...
    def _update_bin_occupancy(self, row):
        ...
        bin_doc.current_occupancy = max(
            (bin_doc.current_occupancy or 0) - row.qty_picked, 0
        )
        bin_doc.save(ignore_permissions=True)
    ...
    def _apply_stock_movement(self, row):
        ...
        frappe.get_doc({
            ...
            "item_code": row.item_code,
            "quantity_change": -row.qty_picked,
            "balance_qty": ledger.balance_qty - row.qty_picked,
            "reserved_qty": max((ledger.reserved_qty or 0) - row.qty_picked, 0),
            "available_qty": (ledger.balance_qty - row.qty_picked) -
                             max((ledger.reserved_qty or 0) - row.qty_picked, 0),
            "stock_uom": row.uom,
            "voucher_type": "WMS Pick List",
            "voucher_no": self.name,
            "is_reservation": 0
